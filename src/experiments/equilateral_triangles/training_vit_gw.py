...
from models.vision_transformer import VisionTransformerGWT
...
if __name__ == "__main__":
    ...
    seeds = config["seeds"]
    workspace_slots = config["num_workspace_slots"]
    ...
    print(
        f"Running GWT-ViT with workspace slots: {workspace_slots}, "
    ...
    raw_path = output_path / f"gwt_raw_results_{experiment_num}.csv"
    history_path = output_path / f"gwt_training_history_{experiment_num}.csv"
    summary_path = output_path / f"gwt_summary_results_{experiment_num}.csv"
    ...
    summary_df = (
        merged_raw_df
        .groupby(["model", "workspace_slots"], dropna=False)
        .agg(
        ...
        .reset_index()
        .sort_values(["model", "workspace_slots"])
    )
